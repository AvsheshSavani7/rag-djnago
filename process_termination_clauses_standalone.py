...
class TerminationClauseProcessor:
    ...
    def get_relevant_chunks(self, field: Dict[str, Any]) -> str:
        ...
        try:
            ...
            search_response_1 = self.index.query(
                ...
                top_k=3,  # Reduced from 10 to 3 as in service.py
                include_metadata=True
            )
            logger.info(
            ...
            search_response_2 = self.index.query(
                ...
                top_k=9,  # Use top_k=9 as in service.py
                include_metadata=True
            )
            logger.info(
    ...
    def extract_field_value_with_gpt(self, clause: Dict[str, Any], context_text: str) -> Dict[str, Any]:
        ...
        # Create dynamic main instruction based on field_name
        main_instruction = f"Determine whether the agreement includes a clause that allows termination based on {field_name}. If such a clause exists, return true. If no such clause exists, return false. Respond strictly with true or false only."
...
If you find the clause but cannot answer a specific field question, include the field in the array but set its answer to "Not found" with 0 confidence."""
    ...
    def _save_results(self):
        ...
        timestamp = ist_now.strftime("%Y%m%d_%I_%M%p")
        ...
        try:
            ...
            # Also save a summary file with same timestamp format
...
def main():
    ...
    processor = TerminationClauseProcessor(max_workers=4)
