#!/usr/bin/env python3
"""
Daily Runner for Riffle Approach 3 (RF3) Twitter Analysis
This script runs daily to process all deals where RF1_approach_done is True.
All Twitter searches are automatically filtered to the last 24 hours using within_time:24h.

Usage:
    python run_daily_riffle_approach_3.py [--dry-run] [--limit N] [--force]
...
Options:
    ...
    --force: Force reprocessing in the analyzer (overrides existing RF3 data)

Note: All Twitter queries automatically include "within_time:24h" filter to only search tweets from the last 24 hours.
"""

from document_processor.twitter_utils.riffle_approach_3 import CompetitiveProductsDealAnalyzer
...
class DailyRiffleApproach3Runner:
    """Daily runner for processing deals with Riffle Approach 3"""
    ...
    def __init__(self, dry_run: bool = False, limit: Optional[int] = None, force: bool = False):
        ...
        if not self.dry_run:
            try:
                self.analyzer = CompetitiveProductsDealAnalyzer()
                self.logger.info("Initialized CompetitiveProductsDealAnalyzer")
            except Exception as e:
                self.logger.error(
                    f"Failed to initialize CompetitiveProductsDealAnalyzer: {e}")
                raise

    def get_eligible_deals(self) -> List[ProcessingJob]:
        """
        Get all deals that are eligible for RF3 processing

        Returns:
            List of ProcessingJob objects that need RF3 processing
        """
        ...
        self.logger.info(
            f"Found {len(deals)} eligible deals for RF3 processing")

        return deals

    def process_deal(self, deal: ProcessingJob) -> Dict[str, Any]:
        """
        Process a single deal with RF3 approach
        ...
        self.logger.info("  Searching tweets from last 24 hours only")
        ...
        try:
            # Run the RF3 analysis with daily_mode=True (automatically filters to last 24 hours)
            result_file = self.analyzer.analyze_deal(
                deal_id, force_reprocess=self.force, daily_mode=True)
            ...
            if result_file == "SKIPPED":
                return {
                    ...
                    'deal_name': deal_name,
                    'status': 'skipped',
                    'message': 'RF3 already completed (skipped)'
                }
            elif result_file:
                return {
                    'deal_id': deal_id,
                    ...
                    'message': 'RF3 processing completed successfully'
                }
            else:
                return {
                    'deal_id': deal_id,
                    'deal_name': deal_name,
                    'status': 'failed',
                    'message': 'RF3 processing failed or no results'
                }
    ...
    def run_daily_processing(self) -> Dict[str, Any]:
        ...
        self.logger.info("=" * 60)
        self.logger.info("Starting Daily RF3 Processing")
        self.logger.info(f"Start time: {start_time}")
        self.logger.info(f"Dry run: {self.dry_run}")
        self.logger.info(f"Force reprocessing: {self.force}")
        self.logger.info("Time filter: Last 24 hours only (within_time:24h)")
        if self.limit:
        ...
        if not eligible_deals:
            self.logger.info("No eligible deals found for RF3 processing")
            return {
                'status': 'completed',
                'total_deals': 0,
                'processed_deals': 0,
        ...
        # Log summary
        self.logger.info("=" * 60)
        self.logger.info("Daily RF3 Processing Summary")
    ...
    def save_summary_report(self, summary: Dict[str, Any]) -> str:
        ...
        filename = f"daily_rf3_report_{timestamp}.json"
...
def setup_logging(log_level: str = 'INFO') -> None:
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler('daily_rf3_runner.log')
        ]
    )


def main():
    """Main function to run the daily RF3 processor"""
    parser = argparse.ArgumentParser(
        description='Daily runner for Riffle Approach 3 Twitter analysis',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Dry run to see what would be processed
  python run_daily_riffle_approach_3.py --dry-run
  
  # Process up to 5 deals (for testing)
  python run_daily_riffle_approach_3.py --limit 5
  
  # Force reprocess all deals (overrides existing RF3 data)
  python run_daily_riffle_approach_3.py --force
  
  # Normal daily run
  python run_daily_riffle_approach_3.py
        """
    )
    ...
    parser.add_argument(
        '--force',
        action='store_true',
        help='Force reprocessing in the analyzer (overrides existing RF3 data)'
    )
    ...
    try:
        # Initialize runner
        runner = DailyRiffleApproach3Runner(
