...
from proxy_processor.agentic_sec_processor import S3Service
...
class ProxySummaryService:
    """Service to generate proxy summary documents using proxy_id"""
    ...
    def get_background_chunks_by_proxy_id(self, proxy_id: str) -> str:
        ...
        try:
            logger.info(
                f"Searching for background chunks (Proxy ID: {proxy_id})")
    ...
    def generate_summary_document(self, proxy_id: str, questions_file: str = None) -> Dict[str, Any]:
        """
        Generate summary document for a proxy document.

        Args:
            proxy_id: The proxy document ID
        ...
        try:
            logger.info(
                f"Starting summary generation for proxy_id: {proxy_id}")

            # Step 1: Get background chunks
            document_text = self.get_background_chunks_by_proxy_id(proxy_id)
            ...
            if questions_file and os.path.exists(questions_file):
                ...
                qa_content = self._process_questions(proxy_id, questions_file)
            ...
            s3_key = f"proxy-summaries/{proxy_id}_summary.docx"
    ...
    def _process_questions(self, proxy_id: str, questions_file: str) -> str:
        ...
        try:
            ...
            # Initialize processor (modified to use proxy_id)
            ...
            for question_key, question in questions_data.items():
                try:
                    # Search for similar chunks using proxy_id
                    results = self._search_chunks_by_proxy_id(
                        processor, question, proxy_id)
    ...
    def _search_chunks_by_proxy_id(self, processor, query: str, proxy_id: str, top_k: int = 8) -> List[Dict]:
        """Search for chunks using proxy_id instead of deal_id"""
        try:
            if processor is None:
            ...
            preamble_results = self._search_preamble_chunks_by_proxy_id(
                processor, query_embedding, proxy_id, top_k
            )
            all_results.extend(preamble_results)

            # Search for general chunks
            general_results = self._search_general_chunks_by_proxy_id(
                processor, query_embedding, proxy_id, top_k
            )
    ...
    def _search_preamble_chunks_by_proxy_id(self, processor, query_embedding: List[float], proxy_id: str, top_k: int = 8) -> List[Dict]:
        """Search for preamble chunks using proxy_id"""
        try:
    ...
    def _search_general_chunks_by_proxy_id(self, processor, query_embedding: List[float], proxy_id: str, top_k: int = 8) -> List[Dict]:
        """Search for general chunks using proxy_id"""
        try:
            if not hasattr(processor, 'index'):
                logger.error("Processor does not have index attribute")
                return []
    ...
    def _create_docx_document(self, proxy_id: str, qa_content: str, merger_background_results: Dict[str, Any]) -> str:
