...
class DOCXFormatter:
    ...
    def add_party_legend(self, legend_text: str):
        """Add a party identification legend before the chronological summary."""
        heading_para = self.doc.add_paragraph()
        heading_run = heading_para.add_run('Party Identification Key')
        heading_run.bold = True
        heading_run.font.size = Pt(12)
        heading_run.font.color.rgb = RGBColor(0, 70, 127)
        ...
        for line in legend_text.strip().split('\n'):
            ...
            if sep_match:
                label_run = para.add_run(sep_match.group(1) + ': ')
                label_run.bold = True
                label_run.font.size = Pt(11)
                desc_run = para.add_run(sep_match.group(2))
...
class ProxyBackgroundAnalyzer:
    ...
    def extract_party_legend(self, extraction_text: str) -> str:
        """Extract a concise party identification legend from the Stage 1 extraction."""
        prompt = """From the merger proxy extraction below, create a concise party identification key listing every named party (Party A, Party B, Company X, Publicis, etc.).

For each party, provide ONE line in this format:
Party A: [brief description — type of company, size, industry, and role in the transaction]

RULES:
1. Include the TARGET company and ACQUIRER as well, not just third parties
2. Include EVERY party mentioned, even if they declined or withdrew early
3. Use the EXACT identifiers from the proxy (Party A, Party B, Company X, etc.)
4. If a party's real name is disclosed (e.g., "Publicis"), use that name
5. Keep each description to one line (under 20 words)
6. Order: Target first, then Acquirer, then other parties alphabetically
7. No bullet points or dashes at the start — just "Party A: description"
...
""" + extraction_text
        ...
        try:
            ...
            legend = response.content[0].text.strip()
            legend = legend.replace('**', '').replace('*', '')
            print(f"    Party legend extracted ({len(legend)} chars)")
            return legend
        except Exception as e:
            print(f"    Warning: party legend extraction failed: {e}")
            return ""
    ...
    def create_document_a(self, extraction_result: Dict, strict_summary_result: Dict, output_path: str, party_legend: str = ""):
