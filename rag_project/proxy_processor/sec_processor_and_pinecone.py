...
class SectionProcessor:
    ...
    def process_flattened_sections(self) -> None:
        ...
        for i, section in enumerate(self.flattened_sections, 1):
            try:
                ...
                # Prepare metadata with proxy information
                metadata = {
                    'title': section['title'],
                ...
                # Upload to Pinecone with proxy-based ID
                proxy_prefix = self.proxy_id.lower().replace(
                    ' ', '_') if self.proxy_id else 'proxy'
                vector_id = f"{proxy_prefix}_{i}"
    ...
    def process_file(self, file_path: str) -> None:
        ...
        try:
            ...
            proxy_name = self.proxy_id.lower().replace(
                ' ', '_') if self.proxy_id else 'proxy'
            output_filename = f'flattened_sections_{proxy_name}.json'
