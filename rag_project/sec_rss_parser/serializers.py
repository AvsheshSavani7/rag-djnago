...
from .models import SECFiling, SECFeedStatus, XBRLFile, LastCronJob
...
class LastCronJobSerializer(serializers.Serializer):
    """Serializer for LastCronJob model"""
    id = serializers.CharField(read_only=True)
    job_name = serializers.CharField(max_length=100)
    last_build_date = serializers.CharField(max_length=255)
    created_at = serializers.DateTimeField(read_only=True)
    updated_at = serializers.DateTimeField(read_only=True)
