"""
S3 helpers for the termination analysis pipeline.

Key pattern:
    termination_analysis/{accession_number}/{doc_type}/{filename}
...
S3_PREFIX = "termination_analysis"

# Ensure the main project .env is loaded (AWS keys live there, not in
# the local Termination_Embeddings_v1/.env).
...
def build_s3_key(accession: str, doc_type: str, filename: str) -> str:
    return f"{S3_PREFIX}/{accession}/{doc_type}/{filename}"


def upload_json(data: dict, accession: str, doc_type: str, filename: str) -> Tuple[str, str]:
    """Upload JSON dict to S3. Returns (s3_key, s3_url)."""
    client, bucket, region = _get_client_and_bucket()
    key = build_s3_key(accession, doc_type, filename)
...
def upload_text(content: str, accession: str, doc_type: str,
                filename: str, content_type: str = "text/plain; charset=utf-8") -> Tuple[str, str]:
...
def download_json(s3_url: str) -> Optional[dict]:
    ...
    req = urllib.request.Request(s3_url, headers={"User-Agent": "TerminationPipeline/1.0"})
