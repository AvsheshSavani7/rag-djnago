...
class SECRSSParser:
    ...
    def fetch_and_parse_html(self, html_url, form_type_from_feed=None):
        ...
        try:
            ...
            if form_type == "8-K":
                has_ex99_1 = any(
                    'EX-99.1' in file.get('type',
                                          '') or 'EX-99.1' in file.get('description', '')
                    for file in xbrl_files
                )
...
class SECFeedProcessor:
    ...
    def save_filing(self, item_data):
        ...
        try:
            ...
            if item_data.get('form_type') == '8-K' and item_data.get('has_ex21'):
            ...
            elif item_data.get('form_type') == '8-K' and item_data.get('has_ex99_1'):
                # Find EX-99.1 file
                ex99_1_files = [
                    file for file in item_data.get('xbrl_files', [])
                ...
                if ex99_1_files:
                    ...
                    # Log the EX-99.1 analysis result
            ...
            # Capture EX-99.1 fields BEFORE allowed_fields filter (they get stripped below)
            ...
            matched_deal = None
            email_type = None  # 'ex21_merger' | 'ex99_1_merger' | None

            if form_type == '8-K' and filing.has_htm_files and item_data.get('document_kind') == 'Definitive Merger Agreement':
                # 8-K EX-2.1 Definitive Merger Agreement – standard email
                should_send_email = True
                email_type = 'ex21_merger'
                logger.info(
                    f"✅ Form type is 8-K (EX-2.1 Definitive Merger) - will send email")
                print(f"✅ Form type is 8-K (EX-2.1 Definitive Merger) - will send email")

            elif form_type == '8-K' and has_ex99_1 is True and is_merger_related is True:
                # 8-K EX-99.1 merger-related – different email (press release / M&A-related)
                should_send_email = True
            ...
            if should_send_email:
                try:
                    ...
                    # Generate email HTML – different template for EX-99.1 merger-related
                    if email_type == 'ex99_1_merger':
                        item_data_for_email = {
                            **item_data,
                            'ex99_1_confidence': ex99_1_confidence,
                            'ex99_1_reasoning': ex99_1_reasoning,
                        }
                    ...
                    payload = {
                        ...
                        # 'ex21_merger' | 'ex99_1_merger' | 'standard'
